...
sum(p3$steps,na.rm=TRUE)# 570,608 steps
summary(p3$steps)#strongly skewed data with more than half of the step counts zero
#initial count of NAs is 2304"
with(p3,table(is.na(p3$steps), p3$date))#by date note some days have all nas
...
aa<-plot(dailysteps_all2[,2], type="l", ylab="Step Count", xlab=NA, main="Average Number of Steps Taken by Time Interval Mean of All Days", xaxt="n")
...
e<-plot(dailysteps_all2[,2], type="l", ylab="Step Count", xlab=NA, main="Average Number of Steps Taken by Time Interval Mean of All Days", xaxt="n")
axis(1,at=c(1,72,144,216,288),las=2,lab = c("12:00am","6:00am","12:00pm","6:00pm","11:55pm"))
...
This plot shows the very small differences between the before and after imputation.
